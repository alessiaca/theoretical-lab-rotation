...
class Unit:
    ...
    #######################################################################################################
    # Initialize the unit
    def __init__(self, type, name=None, tau=20, thres=0, sigma=1, noise_coeff=0):
        ...
        self.tau_noise = 10
        self.eta = 1  # Learning rate (between cue and DLS)
        self.thres_NAc = 0.6
        self.thres_BLA_offset = 0.0001
        self.thres_BLA_trace = 0.4
        self.not_crossed_trace= True
        self.max_weight = 1
        self.activity_history = [[0, 0], [0, 0]]  # Initialize an array that stores the potential and firing rate at each point in time
    ...
    def integrate(self, dt):
        ...
        for connection in self.connections:
            ...
            if connection.type == "plastic":
                ...
                connection.weight = np.clip(connection.weight + weight_change * dt, 0, self.max_weight)
    ...
    def update_firing_rate(self):
        if self.type != "binary":
            ...
            # NEEDS CHANGE FOR INTEGRATION IN THE EMDR MODEL !!!!!!!!!!!!!!!
...
def build_model():
    ...
    # For testing of the goal selection module I implemented the hippocampus (the input to the amygdala) as a binary unit
    # that can switch on an off for testing
    ...
    units["DM_1"] = Unit("leaky", noise_coeff=2); units["DM_2"] = Unit("leaky", noise_coeff=2)
    units["PL_1"] = Unit("leaky", tau=30, sigma=3, thres=0)
    units["PL_2"] = Unit("leaky", tau=30, sigma=3, thres=0)
    ...
    units["DM_1"].add_connections([[units["SNpr_1"], -2.4], [units["DM_2"], -0.3], [units["DM_1"], 1.3]])
    units["DM_2"].add_connections([[units["SNpr_2"], -2.4], [units["DM_1"], -0.3], [units["DM_2"], 1.3]])
